    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      switch(events){
        ...
        case 'LAYTABLE_EXPORT': // 导出
          if (!data.length) return layer.tips('No data in the current table', this, {tips: 3});
          if(device.ie){
            layer.tips('The export function does not support IE, please use Chrome', this, {
              tips: 3
            });
          } else {
            openPanel({
              list: function(){
                return [
                  '<li data-type="csv">Export csv format file</li>',
                  '<li data-type="xls">Export xls format file</li>'
                ].join('')
              }(),
